...
- `commit` command for creating semantic git commits with purpose documentation
  - Invokes `git-commits` skill for formatting guidance
  - Shows git status, staged changes, recent commits via dynamic context
  - Restricted to git, Read, and Skill tools
...
- `exe:list` command for listing execution plans
- `exe:stage` command for staging execution plans
- `plan:create` command for creating plans
