...
1. Read @skills/sub-agent-builder/SKILL.md for overview
2. Read @skills/sub-agent-builder/references/subagents.md for configuration, model selection, tool
   security
3. Read @skills/sub-agent-builder/references/writing-subagent-prompts.md for prompt structure and
   quality
4. Read @skills/sub-agent-builder/SKILL.md section on pure XML structure requirements
5. Read the target subagent configuration file
6. Before penalizing any missing section, search entire file for equivalent content under different
   tag names
...
- Does prompt include workflow steps (under any tag like `<workflow>`, `<approach>`,
  `<critical_workflow>`, etc.)?
...
**Why this matters**: Subagent.md files are consumed only by Claude, never read by humans. Pure XML
structure provides ~25% better token efficiency and consistent parsing.
...
**Why this matters**: Breaks parsing, creates ambiguous boundaries, harder for Claude to parse
structure.
...
**Why this matters**: Inconsistent structure makes parsing unpredictable, reduces token efficiency
benefits.
...
**Why this matters**: Tags should convey meaning, not just structure. Semantic tags improve
